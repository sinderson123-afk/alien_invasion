...
class SoundManager:
    ...
    def __init__(self, enabled=True):
        ...
        self.shoot = self._load_or_synth(
            'resource/sounds/blaster.ogg', lambda: _synth_tone(880, 80, 0.3, 'square'))
        self.explosion = self._load_or_synth(
            'resource/sounds/enemy_destroy.ogg', lambda: _synth_noise(200, 0.5))
        self.missile_launch = self._load_or_synth(
            'resource/sounds/missile.wav', lambda: _synth_tone(220, 150, 0.4, 'square'))
        self.levelup = self._load_or_synth(
            'resource/sounds/weapon_change.ogg', lambda: _synth_chord([523, 659, 784], 400, 0.35))
        self.hit = self._load_or_synth(
            'resource/sounds/enemy_attack.ogg', lambda: _synth_tone(120, 250, 0.5, 'square'))
        self.hurt = self._load_or_synth(
            'resource/sounds/enemy_hurt.ogg', lambda: _synth_tone(200, 100, 0.3, 'square'))
    ...
    def play_bgm(self, path='resource/sounds/Departure_From_The_Last_Moon.mp3'):
        """加载并循环播放背景音乐"""
        if not self.enabled:
            return
        try:
    ...
    def play_boss_destroy(self):
        ...
        if not hasattr(self, '_boss_destroy'):
            self._boss_destroy = self._load_or_synth(
                'resource/sounds/boss_destroy.mp3',
