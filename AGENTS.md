...
- **Neural training**: optional `requirements-training.txt`, `neural_pilot.py`, and `NEURAL_PILOT.md`. Practice must use temporary unauthenticated saves and disable uploads. Ranked inference uses only `GameClient` at normal game speed; never copy practice data into live saves. Model artifacts are gitignored in `training_runs/`.
- **CUDA training**: `neural_gpu.py` uses a separate environment with `requirements-training-cuda.txt`. Spawned CPU game workers must each own their temporary save and SDL instance; only the learner owns CUDA. Keep PPO rollouts on-policy (no teacher action mixing), and retain the current champion until independent evaluation supports replacement.
